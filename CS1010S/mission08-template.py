...
def pushup_score(pushup_table, age, pushup):
    rows = read_csv('pushup.csv')
    if pushup > int(rows[0][len(rows[0])-1]):
        pushup = int(rows[0][len(rows[0])-1])
    elif pushup < int(rows[0][1]):
        pushup = int(rows[0][1])
    else:
        pushup = pushup
...
def situp_score(situp_table, age, situp):
    rows = read_csv('situp.csv')
    if situp > int(rows[0][len(rows[0])-1]):
        situp = int(rows[0][len(rows[0])-1])
    elif situp < int(rows[0][1]):
        situp = int(rows[0][1])
    else:
        situp = situp
...
def run_score(run_table, age, run):
    ...
    if run > int(rows[0][len(rows[0])-1]):
        run = int(rows[0][len(rows[0])-1])
    elif run < int(rows[0][1]):
        run = int(rows[0][1])
    if run % 10 == 0:
        run = run
    else:
...
def ippt_results(ippt_table, age, pushup, situp, run):
    score = (int(pushup_score(ippt_table[0],age,pushup)) + int(situp_score(ippt_table[1],age,situp)) + \
             int(run_score(ippt_table[2],age,run)))
    return (score,ippt_award(int(score)))
