...
*SAS may not be able to read the file crab.txt properly since that file has special separation between values; 

*Q1 - (a);
*Simple version where width in X and weight in Y;
proc sgscatter  data = crab;
   plot weight*width
   /group = spine;
run;

* Complex version with additional information (tittle, x-label, etc);
...
*Q7 - (g): Scatter plot of SR vs fitted is given from the code of Q2-(b) above, it's the figure of RStudent vs Predicted Values;
...
* We need to save all the residuals of the model to a another file to check its normality;
proc reg data=crab;
...
* all the data given and adding another two columns of the fitted values ;
...
proc sgscatter data = check;
   plot SR*yhat;
run; *Or we can use the code below which has a reference line y = 0 added;
...
proc reg data=crab alpha = 0.05;
  model weight = width s1 s2;
output out=predict(where=(weight=.)) p=predicted lclm=LCL_Pred uclm=UCL_Pred;
run;
quit;
* Use lclm - uclm: lower and upper bounds of the CI for Mean weight; 
*Check the dataset "Predict" under Libraries --> WORK in the LEFT panel;
