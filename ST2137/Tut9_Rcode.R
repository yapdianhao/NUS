...
# comments: generally, there is positive relationship (quite linear) between width and weight.
# the variance of the weight is quite stable when the width values change.
# for different types of spine: spine = 2 is lower than other two types
...
s1 = c(rep(0,173))
s2 = c(rep(0,173))
s1[which(spine==1)] = 1 # indicator for spine = 1
s2[which(spine==2)] = 1 # indicator for spine = 2
...
#Q2 - (b)
#Model 2: let R choose the indicator for variable spine by itself
...
#Q5 - (e) 
R2 = summary(model1)$r.squared ; R2 # = 0.7917598

#Q6 - (f)
sigma = summary(model1)$sigma ; sigma # = 0.2656428
var = sigma^2; var #this is the estimation of variance of error term.

#Q7 - (g)
SR = rstandard(model1)

which(SR>3|SR< (-3)) # detect outliers in SR

#QQ plot of SR:
qqnorm(SR,datax = TRUE, ylab = "Standardized Residuals", xlab = "Z scores", main = "QQ Plot", pch = 20)
qqline(SR,datax = TRUE)
#comment: there are outliers on both sides. Other than that,
# the two tails are  a bit heavier than normal. Normality assumption might be violated.
# shapiro.test(SR)

#standardized residuals vs fitted:
plot(model1$fitted.values,SR, xlab="fitted weight", ylab= "Standardized Residuals", main = "SR vs Fitted", pch = 20)
abline(h=0, col = "red")
#comment: other than the outliers, the dots fluctuate quite random around 0
# with no trend nor pattern. Hence, constant variance assumption and independent errors assumption are met.
