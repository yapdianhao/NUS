...
data = read.csv("C:/Data/flextime.txt", sep = "", header = TRUE)
...
data = read.csv("C:/Data/gasoline.csv", sep = ",", header = TRUE)
...
data = read.csv("C:/Data/locate.txt", sep = "", header = TRUE)
data
...
# Q3a -  NORMALITY TEST (all 3 groups are normal, since the p-values are all large)
...
#Q3a - NORMALITY CHECKS for residuals  = check if the residuals of ANOVA ~ normal
mcheck=function(obj){
res=obj$resid
fit=obj$fitted
par(mfrow=c(1,2))
...
# Q3b: can use BONFERRONI TEST or TUKEY  TEST, however, Tukey is stronger
# BONFERRONI TEST:
