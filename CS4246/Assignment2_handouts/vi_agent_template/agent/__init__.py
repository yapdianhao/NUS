...
class GridDrivingMDP(object):
    ...
    def get_transition_function(self):
        ...
        if self.transition_function is None :
            ...
            #print(state_vec_dim, action_dim, transition_function.ndim)
            #print(transition_function.shape)
    ...
    def populate_transition_function(self, state, action, function_matrix) :
        ...
        if state[-1] == 1:
            ...
            #print(transition_function_index)
...
def value_iteration(trn_fn, rwd_fn, gamma):
    ...
    '''
    value_fn = np.zeros(rwd_fn.shape[0]) # utility array
    value_fn_prime = np.zeros(rwd_fn.shape[0])
    delta = 0.001 # max error 
    #print(trn_fn)
    policy = np.zeros(rwd_fn.shape[0], dtype = int)
    states = np.zeros(trn_fn.shape[1], dtype = int)
    for i in range(len(states)):
        states[i] = i
    while True:
        sigma = 0
        value_fn = np.array([i for i in value_fn_prime])
        for state in states:
            subsequent_rwd = rwd_fn[state] + (gamma * value_fn)
            total_rwd = matmul(trn_fn[:,state], subsequent_rwd)
            max_val_idx = np.argmax(total_rwd)
            value_fn_prime[state] = total_rwd[max_val_idx]
            policy[state] = max_val_idx
            if abs(value_fn_prime[state] - value_fn[state]) > sigma:
                sigma = abs(value_fn_prime[state] - value_fn[state])
        if sigma <= delta:
            break
