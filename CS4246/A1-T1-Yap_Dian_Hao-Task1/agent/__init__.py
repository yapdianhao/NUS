...
class GeneratePDDL_Stationary :
    ...
    def generateInitString(self) :
        ...
        initString += self.generateDownInitString()
        initString += self.generateUpInitString()
        initString += self.generateForwardInitString()
        initString += self.generateCarPositionsInitString()
        return initString

    def generateUpInitString(self):
        initString = ''
        for i in range(1, self.num_lanes):
            for j in range(1, self.width):
                initString += '(up_next pt{}pt{} pt{}pt{})'.format(j, i, j - 1, i - 1)
        for i in range(1, self.width):
            initString += '(up_next pt{}pt{} pt{}pt{})'.format(i, 0, i - 1, 0)
        return initString

    def generateDownInitString(self):
        initString = ''
        for i in range(self.num_lanes - 1):
            for j in range(1, self.width):
                initString += '(down_next pt{}pt{} pt{}pt{})'.format(j, i, j - 1, i + 1)
        for i in range(1, self.width):
            initString += '(down_next pt{}pt{} pt{}pt{})'.format(i, self.num_lanes - 1, i - 1, self.num_lanes - 1)
        return initString

    def generateForwardInitString(self):
        initString = ''
        for i in range(self.num_lanes - 1):
            for j in range(1, self.width):
                initString += '(forward_next pt{}pt{} pt{}pt{})'.format(j, i, j - 1, i)
        return initString

    def generateCarPositionsInitString(self):
        initString = ''
        for car in self.state.cars:
            initString += '(at pt{}pt{} car{})'.format(car.position.x, car.position.y, car.id)
...
def generateDomainPDDLFile(gen):
    ...
    gen.addAction(name="UP", parameters=(("car", "car"), ("pt1", "gridcell"), ("pt2", "gridcell")), 
                  precondition_string="(and (at ?pt1 ?car) (not (blocked ?pt2)) (up_next ?pt1 ?pt2))",
                  effect_string="(and (not (at ?pt1 ?car)) (at ?pt2 ?car) (not (blocked ?pt1)) (blocked ?pt2))")
    gen.addAction(name="DOWN", parameters=(("car", "car"), ("pt1", "gridcell"), ("pt2", "gridcell")), 
                  precondition_string="(and (at ?pt1 ?car) (not (blocked ?pt2)) (down_next ?pt1 ?pt2))",
                  effect_string="(and (not (at ?pt1 ?car)) (at ?pt2 ?car) (not (blocked ?pt1)) (blocked ?pt2))")
    gen.addAction(name="FORWARD", parameters=(("car", "car"), ("pt1", "gridcell"), ("pt2", "gridcell")), 
                  precondition_string="(and (at ?pt1 ?car) (not (blocked ?pt2)) (forward_next ?pt1 ?pt2))",
                  effect_string="(and (not (at ?pt1 ?car)) (at ?pt2 ?car) (not (blocked ?pt1)) (blocked ?pt2))")
    # add action here
