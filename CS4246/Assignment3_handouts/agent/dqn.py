...
# Tutorial group: 1
##### Collaborators: None
##### Sources: Lecture notes, pytorch tutorial: https://pytorch.org/tutorials/intermediate/reinforcement_q_learning.html
...
class BaseAgent(Base):
    def act(self, state, epsilon=0.0):
        ...
        '''
        randomized_probability = np.random.uniform()
        if randomized_probability <= epsilon:
            return np.random.randint(self.num_actions)
        else:
            with torch.no_grad():
                output_action = self(state)
                max_idx_tensor = torch.argmax(output_action)
                return max_idx_tensor.item()
...
def compute_loss(model, target, states, actions, rewards, next_states, dones):
    state_actions_with_values = model(states).gather(1, actions.long()).squeeze()
    next_state_values = target(next_states).max(1)[0]
    next_state_values -= torch.mul(dones.squeeze(dim = 1), next_state_values)
    expected_next_state_values = next_state_values * gamma + rewards.squeeze(dim = 1)
    loss = F.smooth_l1_loss(state_actions_with_values, expected_next_state_values)
    return loss
